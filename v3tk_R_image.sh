...
#  1) v3tk_observed_R_image.py  -> XXX_observed_R.png/.pdf
#  2) v3tk_get_legacy.py        -> XXX_legacy_reprojected.jpg
#  3) v3tk_combined_R_image.py  -> XXX_combined_R.png

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ROOT_DIR"

timestamp="$(date +%Y%m%d_%H%M%S)"
logfile="v3tk_R_image_${timestamp}.log"
...
# You can override: MAX_DL=1 ./v3tk_R_image.sh
MAX_DL="${MAX_DL:-2}"
...

{
	...
	"${PY_RUN[@]}" v3tk_observed_R_image.py --input-dir . --overwrite --workers "$NCPU"
	echo
	...
	"${PY_RUN[@]}" v3tk_combined_R_image.py --overwrite --workers "$NCPU"
	echo
